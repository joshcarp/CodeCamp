...
class UnitTest:
    ...
    def run(self, *args, **kwargs):
        ...
        if args and kwargs:
            f_pstr = f"{self.func}({self.args2str(args)}, {self.kwargs2str(kwargs)})"
        elif args:
            f_pstr = f"{self.func}({self.args2str(args)})"
        elif kwargs:
            f_pstr = f"{self.func}({self.kwargs2str(kwargs)})"
        else:
            f_pstr = f"{self.func}()"
        ...
        if not self._kw_handler_('noPrint', True, **kwargs):
            ...
            # handle printing of types accuractely
